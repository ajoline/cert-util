...
cert_files_count=${#cert_files[@]}
  ...
  if [[ $cert_files_count -eq 0 ]]; then
    ...
    if [[ $cert_files_count -eq 1 ]]; then
      ...
      name=$(openssl x509 -noout -subject -in $cert_file | awk -F' ' '{print $3}' | tr -d ',')
    else
  ...
  if [[ $add_san == "y" ]]; then
      read -p "Enter IP address 1 (leave blank for none): " ip1
...
# Generate x509 cert and sign with existing CA
echo ${path}/${domain}.csr
echo ${path}/${name}.crt
echo ${path}/${name}.key
echo ${path}/${domain}.crt
echo ${path}/cert.conf

openssl x509 -req -in ${path}/${domain}.csr -CA ${path}/${name}-CA.crt -CAkey ${path}/${name}-CA.key \
    -CAcreateserial -out ${path}/${domain}.crt \
    -days 365 -sha256 -extfile ${path}/cert.conf \
    2> >(while read line; do echo -ne "."; done)
echo "Successfully created X509 cert for $domain and signed with Certificate authority"
}

# Set your shared config directory
SHARED_CONFIG_DIR='./certs'

# Function to generate the root certificate
generate_root_certificate() {
  certs_generate_root
}

# Function to generate an x509 certificate and sign it with the root certificate
...
main() {
  ...
  while true; do
    echo "1. Generate Root Certificate"
    echo "2. Generate X509 Certificate"
    echo "3. Exit"
